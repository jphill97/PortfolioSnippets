...
namespace Unity.FPS.Game
{
    ...
    public class WeaponController : MonoBehaviour
    {
        ...
        [Tooltip("Will this weapon apply a knockback effect when fired?")]
        public bool KnockBack = false;

        [Tooltip("Magnitude applied to force vector for knockback")]
        ...
        ConcNade activeNade;
        bool canActivateConc = false;

        public GameObject Owner { get; set; }
        public GameObject SourcePrefab { get; set; }
        public GameObject Grapple { get; private set; }
        ...
        public delegate void GrappleHit(Vector3 hitPoint, Transform muzzle);
        public GrappleHit grappleHit;

        public delegate void GrappleUpdate(Vector3 hitPoint);
        public GrappleUpdate grappleUpdate;

        public delegate void GrappleRelease();
        public GrappleRelease grappleRelease;

        public delegate void KnockbackPlayer(Vector3 KnockbackVel, float damage, bool useModifier);
        public KnockbackPlayer knockbackPlayer;
        ...
        public bool HandleShootInputs(bool inputDown, bool inputHeld, bool inputUp)
        {
            ...
            switch (ShootType)
            {
                ...
                case WeaponShootType.Grapple:
                    if (inputDown)
                    {
                        return TryShoot();
                    }

                    if (inputUp && m_CurrentAmmo == 0)
                    {
                        grappleRelease.Invoke();
                        StartCoroutine(ResetGrapple());
                    }

                    return false;

                case WeaponShootType.Conc:
                    if (inputDown && activeNade == null)
                    {
                        return TryShoot();
                    }
                    else if (inputDown && canActivateConc)
                    {
                        activeNade.Explode();
                        activeNade = null;
                        canActivateConc = false;
                    }

                    if (inputUp)
                    {
                        canActivateConc = true;
                    }
        ...
        public void OnGrappleHit(Vector3 hitPoint, GameObject grapple)
        {
            Grapple = grapple;
            grappleHit.Invoke(hitPoint,WeaponMuzzle);
        }

        public void OnConcHit(GameObject concGO)
        {
        ...
        void HandleShoot()
        {
            ...
            for (int i = 0; i < bulletsPerShotFinal; i++)
            {
                ...
                if (ShootType == WeaponShootType.Conc)
                    activeNade = newProjectile.GetComponent<ConcNade>();
            }
            ...
            if (KnockBack)
            {
                knockbackPlayer.Invoke(-WeaponMuzzle.forward * KnockBackAmount, 0f, true);
            }
        ...
        public void GrappleHitWall(Vector3 wallPos)
        {
            grappleUpdate.Invoke(wallPos);
        }
    }
}